...
class ResNet(nn.Module):
    ...
    def forward(self, x, x2, x3, x4, tymode, bboxes=None):
        if tymode == 'train':
            ...
            # guide
            b, c, t, h, w = x[0].size()
            wr1, sr1 = guide(x1, x[0], b, t).__next__() # a,v
            ...
            # x6 = (x[0] + x4[0]) / 2
            wr4, sr4 = guide(x5, x[0], b, t).__next__()
            wr4 = [wr4, ]
            y4, _, _, _ = self.head(wr4)

            # view mutual learning
            sr3 = [sr3, ]
            sr4 = [sr4, ]
            y5, _, _, _ = self.head(sr3)
            y6, _, _, _ = self.head(sr4)
            ...
            y7, alpha3, _, _ = self.head(wr1bn)
            # alph3 = nn.BatchNorm2d(alph3)
            # alph3 = nn.ReLU(alph3)
            y8, alpha4, _, _ = self.head(wr2bn)
        ...
        return y, y3, y5, y6, y7 ,y8, ps_score, ns_score, feature
