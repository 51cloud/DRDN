...
def _get_model_analysis_input(cfg, use_train_input):
    ...
    model_inputs = pack_pathway_output(cfg, input_tensors, input_tensors2)
    model_inputs1 = [model_inputs[0]]
    model_inputs2 = [model_inputs[1]]
    for i in range(len(model_inputs1)):
        model_inputs1[i] = model_inputs1[i].unsqueeze(0)
        if cfg.NUM_GPUS:
            model_inputs1[i] = model_inputs1[i].cuda(non_blocking=True)
    for i in range(len(model_inputs2)):
        model_inputs2[i] = model_inputs2[i].unsqueeze(0)
        if cfg.NUM_GPUS:
            model_inputs2[i] = model_inputs2[i].cuda(non_blocking=True)
    ...
    if cfg.DETECTION.ENABLE:
        ...
        if cfg.NUM_GPUS:
            bbox = bbox.cuda()
        inputs = (model_inputs1, model_inputs2, bbox)
    else:
        inputs = (model_inputs1, model_inputs2)
    return inputs
...
def get_model_stats(model, cfg, mode, use_train_input):
    ...
    if use_train_input:
        # print('inputs', inputs)
        inputs1 = (inputs[0],)
        print('inputs1', inputs[0])
        inputs2 = (inputs[1],)
        print('inputs2', inputs[1])
        count_dict1, _1 = model_stats_fun(model, inputs1)
        #count_dict2, _2 = model_stats_fun(model, inputs2, inputs2)
        count1 = sum(count_dict1.values())
        #count2 = sum(count_dict2.values())
